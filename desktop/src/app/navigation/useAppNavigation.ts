  ...
  const goChannel = React.useCallback(
    (
      ...
      return commitNavigation(
        ...
        // already-active channel (exact re-click is a no-op; jump-to-message
        // /autoSend/force change only search params) never re-run the
        // channel's settle effects, so a trace could only time out — also
        // untraced.
