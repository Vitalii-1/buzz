...
test("DM resurface derives every peer and excludes the signed-in viewer", () => {
  assert.deepEqual(dmPeerPubkeysFromFeedItem(item(), SELF), [ALICE, BOB]);
});
...
test("relay events use the same DM participant and eligibility contract", () => {
  const relayEvent = {
    id: "event-1",
    kind: 40002,
    pubkey: ALICE,
  ...
  assert.deepEqual(dmPeerPubkeysFromRelayEvent(relayEvent, SELF), [ALICE, BOB]);
});
